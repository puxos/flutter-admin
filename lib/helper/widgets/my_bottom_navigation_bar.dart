...
import '../../helper/widgets/my_bottom_navigation_bar_item.dart';
...
class _MyBottomNavigationBarState extends State<MyBottomNavigationBar> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: itemList!.length, initialIndex: widget.initialIndex, vsync: this);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        Expanded(
          child: TabBarView(physics: ClampingScrollPhysics(), controller: _tabController, children: getListOfViews()),
        ),
        Container(
          padding: outerPadding ?? MySpacing.all(16),
          margin: outerMargin ?? MySpacing.zero,
          decoration: containerDecoration ?? BoxDecoration(color: backgroundColor),
