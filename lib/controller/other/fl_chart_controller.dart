...
class FlChartController extends MyController {
  ...
  LineChartBarData get lineChartBarData1_2 => LineChartBarData(
    ...
    isStrokeCapRound: true,
    dotData: const FlDotData(show: false),
    belowBarData: BarAreaData(show: false, color: Colors.pink.withValues(alpha: 0)),
  ...
  LineChartBarData sinLine(List<FlSpot> points) {
    return LineChartBarData(
      spots: points,
      dotData: const FlDotData(show: false),
      gradient: LinearGradient(colors: [Colors.blue.withValues(alpha: 0), Colors.blue], stops: const [0.1, 1.0]),
      barWidth: 4,
      isCurved: false,
    );
  }
  ...
  LineChartBarData cosLine(List<FlSpot> points) {
    return LineChartBarData(
      spots: points,
      dotData: const FlDotData(show: false),
      gradient: LinearGradient(colors: [Colors.pink.withValues(alpha: 0), Colors.pink], stops: const [0.1, 1.0]),
      barWidth: 4,
      isCurved: false,
    );
  }
  ...
  Widget getTitles(double value, TitleMeta meta) {
    final style = TextStyle(color: Colors.blue.shade300, fontWeight: FontWeight.bold, fontSize: 14);
    String text;
    switch (value.toInt()) {
      case 0:
        text = 'Mn';
  ...
  List<BarChartGroupData> get barGroups => [
    BarChartGroupData(x: 0, barRods: [BarChartRodData(toY: 8, gradient: _barsGradient)], showingTooltipIndicators: [0]),
    BarChartGroupData(
      x: 1,
  ...
  List<ScatterSpot> randomData() {
    ...
    return List.generate(blue1Count + blue2Count, (i) {
      return ScatterSpot((Random().nextDouble() * (maxX - 8)) + 4, (Random().nextDouble() * (maxY - 8)) + 4);
    });
  }
  ...
  List<RawDataSet> rawDataSets() {
    return [
      ...
      RawDataSet(title: 'Art & Tech', color: Colors.cyan, values: [250, 100, 200]),
      RawDataSet(title: 'Entertainment', color: Colors.white, values: [200, 150, 50]),
      RawDataSet(title: 'Off-road Vehicle', color: Colors.yellow, values: [150, 200, 150]),
  ...
  List<RadarDataSet> showingDataSets() {
    return rawDataSets().asMap().entries.map((entry) {
      ...
      return RadarDataSet(
        fillColor: isSelected ? rawDataSet.color.withValues(alpha: 0.2) : rawDataSet.color.withValues(alpha: 0.05),
